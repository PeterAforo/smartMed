...
-- This migration addresses the most critical security vulnerabilities without policy conflicts

-- 1. Fix Function Search Path Security Issues (High Priority)
...
-- 2. Add Role Escalation Prevention Trigger (Critical Security Fix)
...
-- Create trigger to prevent privilege escalation (only if it doesn't exist)
...
-- Create trigger for role change auditing (only if it doesn't exist)
